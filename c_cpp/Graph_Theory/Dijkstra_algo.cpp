...
void Dijkstra(const vector<vector<int>> &g, vector<int> &dis,
              vector<bool> &vis) { // O(n^2)
    dis = g[0];
...
// 采用优先队列优化最短距离的查询
void Dijkstra_Heap(const vector<vector<int>> &g, vector<int> &dis,
                   vector<bool> &vis) { // O(n^2)
    int n = dis.size();
    dis.assign(n, X);
    dis[0] = 0; // 源点开始
...
void t1() {
    ...
    vector<int> dis(n);
    vector<bool> vis(n);
    // Dijkstra(adjacent_matrix, dis, vis);
